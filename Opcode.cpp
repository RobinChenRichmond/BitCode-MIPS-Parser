...
// Initializes all the fields for every instruction in Opcode enum
{
  ...
  myArray[ADDI].name = "addi";  myArray[ADDI].numOps = 3;   myArray[ADDI].rdPos = -1;  myArray[ADDI].rsPos = 1;  myArray[ADDI].rtPos = 0;   myArray[ADDI].immPos = 2;  myArray[ADDI].instType = ITYPE; myArray[ADDI].op_field = "001000"; myArray[ADDI].funct_field = "000000";
  ...
  myArray[LB].name = "lb";  myArray[LB].numOps = 3;   myArray[LB].rdPos = -1;  myArray[LB].rsPos = 2;  myArray[LB].rtPos = 0;   myArray[LB].immPos = 1;  myArray[LB].instType = ITYPE; myArray[LB].op_field = "100000"; myArray[LB].funct_field = "000000"; myArray[LB].immBracket = true;
  ...
  myArray[J].name = "j";  myArray[J].numOps = 1;   myArray[J].rdPos = -1;  myArray[J].rsPos = -1;  myArray[J].rtPos = -1;   myArray[J].immPos = 0;  myArray[J].instType = JTYPE; myArray[J].op_field = "000010"; myArray[J].funct_field = "000000"; myArray[J].immLabel = true;
  //Your code here

}

Opcode OpcodeTable::getOpcode(string str, string str2)
...
// template for that instruction.
{
  for(int i = 0; i < (int)UNDEFINED; i++){
    if(myArray[i].op_field == str){
  if(myArray[i].funct_field == str2){
          return (Opcode)i;
  }
    }
  }
...
string OpcodeTable::getName(Opcode o)
  // Given an Opcode, returns its name
{
  if(o < 0 || o >= UNDEFINED){
    return "";
  }
...
// Given an Opcode, returns true if instruction expects a bracket for the register
bool OpcodeTable::isIMMBracket(Opcode o){
  if(o < 0 || o > UNDEFINED)
    return false;
  return myArray[o].immBracket;
}
